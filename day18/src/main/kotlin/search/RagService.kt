...
class RagServiceImpl(
    ...
    override suspend fun answerWithRag(
        ...
        val finalResults = if (filteredReranked != null) {
            // Если был реранкинг, берем topK по rerankScore
            filteredReranked.take(topK).map { it.chunk }
        } else {
            // Если реранкинга не было, берем topK по similarity
            filteredSearch?.take(topK)?.map { it.chunk } ?: emptyList()
        }
        ...
        val contextText = buildContext(finalResults)
