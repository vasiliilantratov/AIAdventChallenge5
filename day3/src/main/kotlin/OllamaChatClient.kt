...
class OllamaChatClient(
    ...
    fun sendMessage(userText: String): String {
        try {
            ...
            val request = ChatCompletionRequest(
                ...
                max_tokens = 2048 // Увеличено для длинных JSON-ответов
            )
    ...
    /**
     * Устанавливает системное сообщение (заменяет существующее)
     */
    fun setSystemMessage(content: String) {
        // Удаляем старое системное сообщение
        chatHistory.removeAll { it.role == "system" }
        // Добавляем новое
        chatHistory.add(0, ChatMessage(role = "system", content = content))
    }
    
    /**
     * Отправляет сообщение пользователя модели с указанным системным сообщением
     * (временно заменяет системное сообщение для этого запроса)
     */
    fun sendMessageWithSystemPrompt(systemPrompt: String, userText: String): String {
        val oldSystemMessage = chatHistory.firstOrNull { it.role == "system" }
        
        try {
            // Временно устанавливаем новое системное сообщение
            setSystemMessage(systemPrompt)
            
            // Отправляем сообщение
            val response = sendMessage(userText)
            
            return response
        } finally {
            // Восстанавливаем старое системное сообщение или очищаем историю
            if (oldSystemMessage != null) {
                setSystemMessage(oldSystemMessage.content)
            } else {
                clearHistory()
            }
        }
