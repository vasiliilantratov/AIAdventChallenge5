...
fun main() = runBlocking {
    ...
    // Инициализируем MCP клиент и запускаем серверы
    val mcpClientManager = McpClientManager()
    var mcpStarted = false
    
    // Запускаем weather server
    println("Запуск MCP Weather Server...")
    val weatherStarted = mcpClientManager.startWeatherServer()
    if (weatherStarted) {
        mcpStarted = true
    ...
    println("Запуск MCP ADB Server...")
    val adbStarted = mcpClientManager.startAdbServer()
    if (adbStarted) {
        mcpStarted = true
        val tools = mcpClientManager.getAvailableTools()
        println("✓ MCP ADB Server запущен. Всего доступных инструментов: ${tools.map { it.name }}")
    } else {
        println("⚠ Не удалось запустить MCP ADB Server. Продолжаем без поддержки Android устройств.")
    }
