    ...
    private fun parseOllamaResponse(responseBody: String): OllamaChatResponse {
        ...
        for (line in lines) {
            try {
                ...
                // Пропускаем некорректные строки, но логируем
                ApiLogger.logError("Ошибка парсинга строки NDJSON: ${e.message}. Строка: ${line.take(100)}")
            }
        }
        
        // Возвращаем финальный ответ с собранным контентом
        val finalMessage = if (lastResponse?.message != null) {
            ...
            OllamaMessage(role = "assistant", content = fullContent.toString())
        }
