...
import org.example.indexing.IgnorePattern
...
class IndexCommand : CliktCommand(name = "index", help = "Index a directory") {
    ...
    val excludePatterns by option("--exclude", help = "Exclude pattern (glob format, can be used multiple times)").multiple()
    val useDefaultExcludes by option("--use-default-excludes", help = "Use default exclude patterns").flag(default = true)
    val useGitignore by option("--use-gitignore", help = "Read and use .gitignore file if present").flag(default = true)
    ...
    override fun run() = runBlocking {
        ...
        if (useGitignore) {
            val gitignorePath = File(path, ".gitignore").absolutePath
        ...
        allExcludePatterns.addAll(excludePatterns)
        
        val ignorePattern = if (allExcludePatterns.isNotEmpty()) {
            IgnorePattern.from(allExcludePatterns)
        } else {
            null
        }
        ...
        if (allExcludePatterns.isNotEmpty()) {
            println("Exclude patterns: ${allExcludePatterns.take(10).joinToString(", ")}${if (allExcludePatterns.size > 10) "..." else ""}")
        }
