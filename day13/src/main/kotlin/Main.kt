...
fun main() = runBlocking {
    ...
    // Инициализируем базу данных подписок на погоду
    val subscriptionDatabase = WeatherSubscriptionDatabase()
    ...
    // Инициализируем сервисы для подписок на погоду
    val clothingService = ClothingRecommendationService()
    val weatherSubscriptionService = if (mcpStarted) {
    ...
    val notificationScheduler = if (weatherSubscriptionService != null) {
        val scheduler = WeatherNotificationScheduler(weatherSubscriptionService, subscriptionDatabase)
        scheduler.start()
        println("✓ Планировщик уведомлений о погоде запущен")
        scheduler
    } else {
        null
    }
    ...
    val chatSession = ChatSession(
        ...
        weatherSubscriptionService = weatherSubscriptionService
    )
