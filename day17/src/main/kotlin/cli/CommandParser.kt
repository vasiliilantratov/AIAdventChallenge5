...
class QaCommand : CliktCommand(name = "qa", help = "Ask a question with or without RAG") {
    ...
    private val topK by option("--top-k", help = "Number of chunks to use for RAG mode")
        .convert { it.toInt() }
        .default(5)
    private val question by argument("question", help = "Question to ask the model")
    ...
    override fun run() = runBlocking {
        ...
        val ragService = RagServiceImpl(semanticSearch, embeddingService, llmService)
        ...
        when (mode) {
            ...
            "rag" -> {
                println("Режим: с RAG (top-k=$topK)")
                println("Вопрос:")
                println(question)
                println()

                val ragAnswer = ragService.answerWithRag(question, topK)
                println("Ответ:")
